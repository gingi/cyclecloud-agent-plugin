...
- **Preparation:** use a clean checkout of the default branch to prepare version changes and release notes, then create one branch commit and PR through GitHub's API. The checkout is not modified, and no tag or public release is created. Requests refuse existing tags and orphan branches instead of overwriting them; an existing closed release PR is reported without being reopened.
