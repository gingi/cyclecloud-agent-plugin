...
The development workflow verifies branches and PRs, packages the plugin, records source/checkout identity, and uploads a 14-day Actions artifact. `npm run release:prepare -- <version>` updates local manifest versions and prepares the corresponding top-level `CHANGELOG.md` entry without committing; the MCP handshake version is bundled from `package.json`. Changelog entries use `## [version]` headings. Preparation promotes curated Unreleased notes or, for hosted PR requests, uses generated notes when pending notes are empty. Existing version entries and older history are preserved. Publication extracts only the selected version's nonempty entry; separate per-release documentation files are not used.

The **Prepare or preview release** workflow defaults to `mode=request`. On the default branch, it uses `release:request` to create a `release/v<version>` branch and PR containing the version and changelog changes. Repeat requests reuse an open PR without rewriting reviewer edits. An optional scoped GitHub App token allows automatic downstream PR checks; the built-in token fallback can require a maintainer to approve workflow execution. Repository branch rules provide the required-check and review policy.

The **Release** workflow runs only after a same-repository release PR merges into the default branch. Its shared **Build and publish release** workflow requires either a current non-author human owner/member/collaborator approval or a deliberate human merge (including self-merge), rejects outstanding changes-requested reviews, and validates versions and the selected changelog entry at the recorded merge SHA. It verifies, packages, publishes, and checks curl installation from that exact commit.

The same reusable workflow handles explicit `mode=preview` dispatches from non-default branches. Preview mode requires a prerelease version and a clean checkout matching the dispatch SHA, with matching committed manifests and changelog notes. It creates no PR and makes no changes to `main`. It always publishes as a prerelease with `latest=false`, runs version-pinned public curl checks, and retains the source branch. A branch-local version of the existing dispatchable workflow can call a branch-local reusable workflow without first merging those changes into `main`. Direct tag pushes are not publication triggers. Existing tags/releases are never overwritten.

A read-only post-release job exercises anonymous curl installation from the actual public release assets (and the latest link when applicable), verifies installed version/commit metadata and both runtime copies, then initializes the bundled MCP server and lists its read-only tools. It uses a fake Copilot CLI and temporary credentials, not a personal login or a real CycleCloud connection. Failure marks the workflow red but does not delete the already-published release. After a reviewed release succeeds, cleanup deletes only the unchanged preparation branch using a head-SHA lease, retaining branches with newer work or another open PR. Preview dispatches never run branch cleanup. `npm run package:release -- v<version>` builds assets locally; `npm run verify:release -- <tag> <commit>` rechecks a published release. See the [release guide](development.md#publish-a-release).
