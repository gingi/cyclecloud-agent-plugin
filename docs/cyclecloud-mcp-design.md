...
The **Release** workflow runs only after a same-repository release PR merges into the default branch. Its shared **Build and publish release** workflow requires a current non-author human owner/member/collaborator approval, rejects outstanding changes-requested reviews, and validates versions and the selected changelog entry at the recorded merge SHA. It verifies, packages, publishes, and checks curl installation from that exact commit.
