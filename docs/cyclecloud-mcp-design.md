...
The development workflow verifies branches and PRs, packages the plugin, records source/checkout identity, and uploads a 14-day Actions artifact. `npm run release:prepare -- <version>` fetches origin/tags, creates `release/prepare-<version>` at the current commit, updates local manifest versions, and drafts a version-specific changelog entry from non-merge commit subjects since the nearest reachable SemVer release tag. Mechanical release commits are filtered out. Preparation returns with uncommitted changes for review and editing; reruns on the matching branch preserve existing notes and other edits. Maintainers commit the reviewed changes and merge an ordinary PR for stable releases. `npm run release:tag -- <version>` validates the selected clean checkout, requires stable commits to belong to the origin default branch, runs verification, and creates an annotated tag when none exists. Existing annotated or lightweight tags are reused unchanged only when they identify the selected commit. Adding `--push` publishes only that tag; prereleases may be tagged directly from a reviewed branch commit. The MCP handshake version is bundled from `package.json`.
