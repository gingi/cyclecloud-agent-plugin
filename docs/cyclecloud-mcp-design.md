...
The development workflow verifies branches and PRs, packages the plugin, records source/checkout identity, and uploads a 14-day Actions artifact. `npm run release:prepare -- <version>` updates local manifest versions without committing; the MCP handshake version is bundled from `package.json`.

The **Prepare release PR** workflow accepts a version on the default branch and uses `release:request` to create a `release/v<version>` branch and PR. A single GitHub API commit contains the version changes and generated, reviewable notes at `docs/releases/v<version>.md`; the checkout remains unchanged. Repeat requests reuse an open PR without rewriting reviewer edits. An optional scoped GitHub App token allows automatic downstream PR checks; the built-in token fallback can require a maintainer to approve workflow execution. Repository branch rules provide the required-check and review policy.

The **Release** workflow runs only after a same-repository release PR merges into the default branch. It requires a current non-author human owner/member/collaborator approval, rejects outstanding changes-requested reviews, and validates versions and reviewed notes in the exact recorded merge commit. It builds a tarball, version-pinned curl bootstrap, and checksums, and smoke-tests the extracted archive. A separate write-permission job creates/verifies the tag at that merge SHA and publishes persistent assets with the reviewed notes. Direct tag pushes and independent manual publish dispatches are not supported. Existing tags/releases are not overwritten; prereleases do not replace latest stable.

A read-only post-release job exercises anonymous curl installation from the actual public release assets (and the latest link when applicable), verifies installed version/commit metadata and both runtime copies, then initializes the bundled MCP server and lists its read-only tools. It uses a fake Copilot CLI and temporary credentials, not a personal login or a real CycleCloud connection. Failure marks the workflow red but does not delete the already-published release. After success, cleanup deletes only the unchanged preparation branch using a head-SHA lease, retaining branches with newer work or another open PR. `npm run package:release -- v<version>` builds assets locally; `npm run verify:release -- <tag> <commit>` rechecks a published release. See the [release guide](development.md#publish-a-release).
