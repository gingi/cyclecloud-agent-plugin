...
export async function createCycleCloudClient(
    settings: CycleCloudSettings,
...
async function createDispatcher(settings: CycleCloudSettings): Promise<Agent> {
    ...
    const ca =
        settings.caCertPath === undefined
            ? [...rootCertificates]
            : [...rootCertificates, await loadCustomCa(settings.caCertPath)];
    return new Agent({
        connections: 4,
        connect: {
...
function validateCaStats(stats: Stats, effectiveUserId: number): void {
    if (
        ...
        !stats.isFile() ||
        (stats.uid !== 0 && stats.uid !== effectiveUserId)
    ) {
        throw new StartupError("configuration_invalid", "invalid_ca_path");
    }
