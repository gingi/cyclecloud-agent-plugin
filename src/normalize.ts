...
export function normalizeClusterList(raw: unknown, limit: number): ClusterListResult {
  validateLimit(limit, 1, 200);
  if (!Array.isArray(raw)) invalidResponse();

  const clusters = raw.map(normalizeClusterSummary).sort((left, right) => compareNames(left.name, right.name));
...
export function normalizeCluster(raw: unknown, requestedName: string, fixedNodeLimit: number, nodeArrayLimit: number): ClusterDetailResult {
  ...
  return {
    cluster: {
      ...
      fixedNodeDefinitionsTruncated: returnedFixedNodes.length < fixedNodes.length,
      arrayNodeCount,
      configuredNodeCount,
    },
  };
...
export function normalizeClusterStatus(
  ...
  const nodeArrays = record.nodearrays.map(normalizeNodeArrayStatus).sort((left, right) => compareNames(left.name, right.name));
  ...
  const returnedNodeArrays = selectedNodeArrays.map((nodeArray): NodeArrayStatus => {
...
function normalizeNodeArray(value: unknown): NodeArraySummary {
  ...
  const coreCount = coreCountValue === undefined ? undefined : requiredNonNegativeInteger(coreCountValue);
  return {
    template,
    ...optionalProperty("state", state),
    ...optionalProperty("targetState", targetState),
...
function normalizeNodeArrayStatus(value: unknown): Omit<NodeArrayStatus, "bucketTotal" | "bucketReturned" | "bucketsTruncated"> {
  ...
  return {
    ...
    buckets: record.buckets.map(normalizeBucketStatus).sort((left, right) => compareNames(left.bucketId, right.bucketId)),
  };
}

function normalizeBucketStatus(value: unknown): BucketStatus {
  const record = requiredRecord(value);
  const definition = record.definition === undefined ? undefined : requiredRecord(record.definition);
  const machineType = definition === undefined ? undefined : optionalWireString(definition.machineType, 256);
  const invalidReason = optionalWireString(record.invalidReason, 256);
  const lastCapacityFailure = optionalFiniteNumber(record.lastCapacityFailure);
  const spotPlacementScore = optionalWireString(record.spotPlacementScore, 256);
  ...
  return {
    ...
    availableCoreCount: requiredNonNegativeInteger(record.availableCoreCount),
    ...optionalProperty("lastCapacityFailure", lastCapacityFailure),
    ...optionalProperty("spotPlacementScore", spotPlacementScore),
  };
}

function consumedFields(value: unknown, consumedNames: readonly string[]): ConsumedFields {
  const record = requiredRecord(value);
...
function optionalArray(fields: ConsumedFields, name: string): readonly unknown[] {
...
function requiredString(fields: ConsumedFields, name: string, maximumScalars: number): string {
  return requiredWireString(requiredField(fields, name), maximumScalars);
}

function optionalString(fields: ConsumedFields, name: string, maximumScalars: number): string | undefined {
...
function optionalWireString(value: unknown, maximumScalars: number): string | undefined {
  if (value === undefined) return undefined;
  if (typeof value !== "string" || !isWellFormed(value) || /[\u0000-\u001f\u007f-\u009f]/u.test(value)) {
    invalidResponse();
  }
...
function optionalProperty<Value>(key: string, value: Value | undefined): Record<string, Value> {
