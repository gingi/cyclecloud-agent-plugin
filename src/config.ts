...
export async function loadConfiguration(
    ...
    const settings: CycleCloudSettings =
        document.caCertPath === undefined
            ? commonSettings
            : { ...commonSettings, caCertPath: document.caCertPath };
...
function parseDocument(contents: string): ConfigurationDocument {
    ...
    if (
        result.data.caCertPath !== undefined &&
        !isAbsolute(result.data.caCertPath)
    ) {
...
function validateUrlAndTransport(document: ConfigurationDocument): void {
    ...
    const validHttp =
        ...
        document.verifyTls &&
        !hasCa &&
        ((loopback && !document.allowInsecureHttp) ||
            (!loopback && document.allowInsecureHttp));
    const validVerifiedHttps =
        url.protocol === "https:" &&
        document.verifyTls &&
        !document.allowInsecureHttp;
