...
const clusterNameSchema = z
  ...
  .refine((value) => value.length > 0 && [...value].length <= 256, "Cluster name must contain 1 through 256 characters.")
  .refine(isWellFormed, "Cluster name must be well-formed Unicode.")
  .refine((value) => !containsControlCharacter(value), "Cluster name must not contain control characters.")
  .refine((value) => value !== "." && value !== "..", "Cluster name must not be a dot segment.");
const listInputSchema = z.object({ limit: z.number().int().min(1).max(200).default(50) }).strict();
...
export function createCycleCloudMcpServer(options: CreateCycleCloudMcpServerOptions): McpServer {
  ...
  server.registerTool(
    "list_clusters",
    {
      description: "List non-template CycleCloud clusters using bounded structured output.",
  ...
  server.registerTool(
    "get_cluster",
    {
      description: "Get bounded CycleCloud cluster details for one exact cluster name.",
...
function errorResult(error: unknown): CallToolResult {
  const requestError = error instanceof CycleCloudRequestError ? error : new CycleCloudRequestError("invalid_response", false);
...
function mutationSummary(action: "start" | "terminate", outcome: "accepted" | "unknown"): string {
