import type { ActionDispatchResult, CycleCloudClient } from "./cyclecloud-client.js";
...
export const unknownOutcomeWarning = "The CycleCloud action may have partially completed. Inspect the cluster status before retrying.";
export const followUpWarning = "The CycleCloud action was accepted, but the follow-up status read did not complete.";
...
export class CycleCloudTools {
  ...
  async listClusters(input: ListClustersInput, signal: AbortSignal): Promise<ClusterListResult> {
    return normalizeClusterList(await this.#client.listClusters({ signal }), input.limit);
  }

  async getCluster(input: GetClusterInput, signal: AbortSignal): Promise<ClusterDetailResult> {
  ...
  async getClusterStatus(input: GetClusterStatusInput, signal: AbortSignal): Promise<ClusterStatusResult> {
  ...
  async startCluster(input: MutationInput, signal: AbortSignal): Promise<MutationResult> {
    return this.#mutate("start", input, signal);
  }

  async terminateCluster(input: MutationInput, signal: AbortSignal): Promise<MutationResult> {
    return this.#mutate("terminate", input, signal);
  }

  async #mutate(action: "start" | "terminate", input: MutationInput, signal: AbortSignal): Promise<MutationResult> {
    ...
    try {
      ...
      try {
        ...
        const hasObservedStatus = status.state !== undefined || status.targetState !== undefined;
        return {
          action,
          ...input,
          outcome: "accepted",
          ...(hasObservedStatus
            ? {
                observedStatus: {
                  ...
                  ...(status.targetState === undefined ? {} : { targetState: status.targetState }),
                },
              }
            : {}),
        };
  ...
  async #dispatch(action: "start" | "terminate", input: MutationInput, signal: AbortSignal): Promise<ActionDispatchResult> {
    return action === "start"
      ? this.#client.startCluster(input.clusterName, input.recursive, { signal })
      : this.#client.terminateCluster(input.clusterName, input.recursive, { signal });
  }
}
