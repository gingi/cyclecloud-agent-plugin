...
function settings(
    url: string,
    overrides: Partial<CycleCloudSettings> = {},
): CycleCloudSettings {
    return {
        url,
        verifyTls: true,
...
async function createClient(
    configuration: CycleCloudSettings,
): Promise<CycleCloudClient> {
...
describe("CycleCloud HTTP request shape", () => {
    test("Uses exact routes, encoded names, queries, and Basic authorization", async () => {
        ...
        for (let index = 0; index < 3; index += 1)
            server.enqueue({ status: 200, body: index === 2 ? "{}" : "[]" });
    ...
    test("Does not follow redirects or forward credentials", async () => {
        ...
        server.enqueue({
            status: 302,
            headers: { location: `${server.origin}/redirect-target` },
            body: "redirect",
        });
        const client = await createClient(settings(server.origin));
...
describe("CycleCloud HTTP error handling", () => {
    test.each([
        ...
        server.enqueue({
            status,
            body: "server-controlled secret instruction",
        });
        const client = await createClient(settings(server.origin));
        ...
        expect(error.message).not.toMatch(
            /secret|instruction|server-controlled/i,
        );
        expect(server.requests).toHaveLength(1);
    });
    ...
    test("Rejects malformed and oversized successful JSON", async () => {
        ...
        server.enqueue({
            status: 200,
            body: JSON.stringify({ value: "x".repeat(8 * 1024 * 1024) }),
        });
        const client = await createClient(settings(server.origin));
    ...
    test("Enforces the response limit after decompression", async () => {
        ...
        const oversized = gzipSync(
            Buffer.from(JSON.stringify({ value: "x".repeat(8 * 1024 * 1024) })),
        );
        server.enqueue({
            status: 200,
            body: oversized,
    ...
    test("Maps read timeout and pre-dispatch cancellation without exposing exceptions", async () => {
        ...
        const client = await createClient(
            settings(server.origin, { requestTimeoutMs: 20 }),
        );

        const timeout = await expectRequestError(
            client.listClusters(),
            "timeout",
            true,
        );
        ...
        controller.abort();
        await expectRequestError(
            client.listClusters({ signal: controller.signal }),
            "cancelled",
            false,
        );
        expect(server.requests).toHaveLength(1);
    ...
    test("Classifies lifecycle responses without retrying", async () => {
        ...
        await expect(client.startCluster("cluster-1", false)).resolves.toEqual({
            outcome: "accepted",
        });
        await expect(
            client.terminateCluster("cluster-1", true),
...
describe("CycleCloud TLS", () => {
    test("Extends default trust with a permission-checked custom CA and enforces SAN", async () => {
        ...
        const trusted = await createClient(
            settings(server.origin, { caCertPath: pki.caPath }),
        );
        await expect(trusted.listClusters()).resolves.toEqual([]);

        const defaultTrust = await createClient(settings(server.origin));
        await expectRequestError(
            defaultTrust.listClusters(),
            "tls_error",
            false,
        );

        const mismatchedHost = await createClient(
            settings(server.origin.replace("localhost", "127.0.0.1"), {
                caCertPath: pki.caPath,
            }),
        );
        await expectRequestError(
            mismatchedHost.listClusters(),
            "tls_error",
            false,
        );
    });
    ...
    test("Rejects an insecure custom CA file before making a request", async () => {
        ...
        const error = await createCycleCloudClient(
            settings("https://localhost", { caCertPath: pki.caPath }),
            credentials,
        ).catch((caught: unknown) => caught);
        expect(error).toBeInstanceOf(StartupError);
        expect(error).toMatchObject({
            code: "configuration_invalid",
            reason: "invalid_ca_path",
        });
    });
});
