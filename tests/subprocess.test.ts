...
import { startFakeCycleCloudServer, type FakeCycleCloudServer } from "./helpers/fake-cyclecloud.js";
...
const manifest = manifestSchema.parse(JSON.parse(readFileSync(join(repositoryRoot, "mcp.json"), "utf8")));
...
async function createPluginRoot(): Promise<string> {
  ...
  await Promise.all([
    copyFile(join(repositoryRoot, "plugin.json"), join(pluginRoot, "plugin.json")),
    copyFile(join(repositoryRoot, "mcp.json"), join(pluginRoot, "mcp.json")),
...
function serverArgs(pluginRoot: string): string[] {
  return configuredArgs.map((argument) => argument.replace("${PLUGIN_ROOT}", pluginRoot));
}

function childEnvironment(pluginRoot: string, pluginData: string): Record<string, string> {
  return {
...
async function snapshotPluginRoot(pluginRoot: string): Promise<readonly string[]> {
...
async function writeConfiguration(pluginData: string, url: string, username: string, password: string): Promise<void> {
...
describe("Bundled stdio MCP server", () => {
  ...
  test("Reports missing configuration only on stderr without changing the plugin root", async () => {
    ...
    const pluginData = await createTemporaryDirectory("cyclecloud-mcp-data-");
  ...
  test("Captures a real MCP session and exposes no credential on stdout, stderr, or results", async () => {
    ...
    servers.push(server);
    server.enqueue({ status: 200, body: JSON.stringify([{ ClusterName: "cluster-1" }]) });
    ...
    const transport = new StdioClientTransport({
      ...
      args: [captureWrapper, capturePath, command, ...serverArgs(pluginRoot)],
      env: childEnvironment(pluginRoot, pluginData),
      stderr: "pipe",
    });
    let stderr = "";
    ...
    const client = new Client({ name: "cyclecloud-mcp-subprocess-test", version: "0.1.0" });
    ...
    try {
      ...
      result = await client.callTool({ name: "list_clusters", arguments: {} });
      expect(tools.tools.map((tool) => tool.name)).toEqual(["list_clusters", "get_cluster", "get_cluster_status"]);
    ...
    const encoded = Buffer.from(`${username}:${password}`, "ascii").toString("base64");
