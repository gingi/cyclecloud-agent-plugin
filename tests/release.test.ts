...
async function setVersion(version: string) {
    ...
    const marketplacePath = join(workspace, ".github/plugin/marketplace.json");
...
describe("Release packaging", () => {
    test("Checks a version tag without requiring a built package", () => {
        const result = run("--check", "v0.1.0");
    ...
    test.each([
        ...
        const result = run("--check", tag);
        expect(result.status).not.toBe(0);
        expect(result.stderr).toMatch(/tag|version/i);
    });

    test("Accepts an explicitly versioned prerelease", async () => {
        await setVersion("0.2.0-rc.1");
        const result = run("--check", "v0.2.0-rc.1");
        expect(result.status, result.stderr).toBe(0);
    });
