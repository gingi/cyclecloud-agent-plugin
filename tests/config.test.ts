...
describe("CycleCloud configuration", () => {
    test("Applies secure defaults and separates credentials from settings", async () => {
        ...
        expect(JSON.stringify(loaded.settings)).not.toMatch(
            /username|password|correct horse/i,
        );
        expect(loaded.credentials.getCredentials()).toEqual({
            username: "cyclecloud-poc",
            password: "correct horse battery staple",
        });
        expect(loaded.credentials.getCredentials()).not.toBe(
            loaded.credentials.getCredentials(),
        );
    });
    ...
    test.each([
        ...
        [
            "short action timeout",
            { actionTimeoutMs: 34_999 },
            "invalid_timeout",
        ],
        [
            "long action timeout",
            { actionTimeoutMs: 300_001 },
            "invalid_timeout",
        ],
        [
            "non-boolean mutation gate",
            { enableMutations: "true" },
            "invalid_boolean",
        ],
    ...
    test.each([
        ...
        [
            "credentials in URL",
            { url: "https://user:pass@cyclecloud.example.com" },
        ],
    ...
    test.each([
        [
            "remote HTTP without opt-in",
            { url: "http://cyclecloud.example.com" },
        ],
        [
            "HTTP with disabled TLS flag",
            { url: "http://127.0.0.1", verifyTls: false },
        ],
        [
            "HTTP with a CA",
            { url: "http://127.0.0.1", caCertPath: "/tmp/ca.pem" },
        ],
        [
            "loopback HTTP with redundant opt-in",
            { url: "http://127.0.0.1", allowInsecureHttp: true },
        ],
        ["remote HTTPS with disabled verification", { verifyTls: false }],
        [
            "disabled verification with a CA",
            {
                url: "https://127.0.0.1",
                verifyTls: false,
        ...
        "Rejects invalid transport combination %s",
    ...
    test.each([
        ...
        [
            ...
            { url: "http://cyclecloud.example.com", allowInsecureHttp: true },
        ],
        ["verified HTTPS", {}],
        [
            "loopback HTTPS without verification",
            { url: "https://[::1]", verifyTls: false },
        ],
    ...
    test.each([0o644, 0o700, 0o200, 0o4600])(
        "Rejects credential mode %s",
        async (mode) => {
    ...
    test("Rejects a symlinked credential file", async () => {
        ...
        await writeFile(target, JSON.stringify(validDocument()), {
            mode: 0o600,
        });
    ...
    test("Rejects a credential file not owned by the effective user", async () => {
        ...
        await expectStartupError(
            loadConfiguration({ pluginData, effectiveUserId: stats.uid + 1 }),
            "credential_file_insecure",
            "wrong_owner",
        );
    });
