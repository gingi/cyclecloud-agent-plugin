...
afterEach(async () => {
  await Promise.allSettled(connections.splice(0).flatMap(({ client, server }) => [client.close(), server.close()]));
});
...
async function connectServer(options: {
  ...
  const client = new Client({ name: "cyclecloud-mcp-test", version: "0.1.0" });
...
describe("CycleCloud MCP discovery", () => {
  test("Registers exactly three read tools by default with read-only annotations", async () => {
    ...
    expect(listed.tools.map((tool) => tool.name)).toEqual(["list_clusters", "get_cluster", "get_cluster_status"]);
    ...
    expect(listed.tools.every((tool) => tool.annotations?.openWorldHint === true)).toBe(true);
    const absentMutation = await client.callTool({ name: "start_cluster", arguments: { clusterName: "cluster-1" } });
  ...
  test("Registers both mutation tools only when enabled and emits one warning", async () => {
    ...
    const { client } = await connectServer({ cycleCloud, enableMutations: true, onWarning: (warning) => warnings.push(warning) });
    ...
    const mutations = listed.tools.filter((tool) => tool.name.endsWith("_cluster") && !tool.name.startsWith("get_"));
    expect(mutations.every((tool) => tool.annotations?.readOnlyHint === false)).toBe(true);
    expect(mutations.every((tool) => tool.annotations?.destructiveHint === true)).toBe(true);
    expect(mutations.every((tool) => tool.annotations?.idempotentHint === false)).toBe(true);
...
describe("CycleCloud MCP calls", () => {
  test("Returns structured bounded read content and applies input defaults", async () => {
    ...
    expect(result.structuredContent).toMatchObject({ total: 1, returned: 1, truncated: false });
    expect(result.content).toEqual([{ type: "text", text: "Returned 1 of 1 CycleCloud clusters." }]);
  ...
  test("Trims and validates cluster names before calling CycleCloud", async () => {
    ...
    await expect(client.callTool({ name: "get_cluster", arguments: { clusterName: "  cluster-1  " } })).resolves.toMatchObject({
      isError: false,
    });
    const dotSegment = await client.callTool({ name: "get_cluster", arguments: { clusterName: ".." } });
    const controlCharacter = await client.callTool({ name: "get_cluster", arguments: { clusterName: "bad\nname" } });
  ...
  test("Rejects out-of-range and undeclared inputs before calling CycleCloud", async () => {
    const cycleCloud = new FakeCycleCloudClient();
    const { client } = await connectServer({ cycleCloud, enableMutations: true });

    const listLimit = await client.callTool({ name: "list_clusters", arguments: { limit: 201 } });
    ...
    expect([listLimit, nodeArrayLimit, undeclaredMutationInput].every((result) => result.isError === true)).toBe(true);
    expect(cycleCloud.calls).toMatchObject({ list: 0, status: 0, start: 0 });
  });
  ...
  test("Returns exact fixed structured errors without server text", async () => {
    ...
    expect(result.structuredContent).toEqual({
      error: {
        ...
        message: "CycleCloud returned a response the plugin could not safely use.",
        retryable: false,
      },
    });
    expect(result.content).toEqual([{ type: "text", text: "CycleCloud returned a response the plugin could not safely use." }]);
  });
