...
export async function startFakeCycleCloudServer(
  ...
  const handler = (request: IncomingMessage, response: ServerResponse): void => {
    ...
    const queued = responses.shift() ?? { status: 500, body: "No queued response" };
    const send = (): void => {
  ...
  const server =
    options.tls === undefined
      ...
      : createHttpsServer({ cert: options.tls.certificate, key: options.tls.privateKey }, handler);
...
export async function createTestPki(directory: string, commonName = "localhost"): Promise<TestPki> {
  ...
  await writeFile(extensionsPath, `subjectAltName=DNS:${commonName}\nextendedKeyUsage=serverAuth\n`);
  await run("openssl", [
    "x509",
    "-req",
    "-in",
