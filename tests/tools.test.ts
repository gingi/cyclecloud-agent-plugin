...
const status = { state: "Started", targetState: "Started", nodearrays: [], maxCount: 10, maxCoreCount: 20 };
...
describe("CycleCloud read tools", () => {
  test("Normalize bounded list, detail, and status results", async () => {
    ...
    await expect(tools.listClusters({ limit: 50 }, openSignal)).resolves.toMatchObject({ total: 1, returned: 1 });
    await expect(tools.getCluster({ clusterName: "cluster-1", fixedNodeLimit: 50, nodeArrayLimit: 50 }, openSignal)).resolves.toMatchObject(
      { cluster: { name: "cluster-1" } },
    );
    await expect(
      tools.getClusterStatus({ clusterName: "cluster-1", nodeArrayLimit: 20, bucketLimit: 20 }, openSignal),
    ).resolves.toMatchObject({ status: { clusterName: "cluster-1", state: "Started" } });
  ...
  test("Propagate fixed request errors unchanged", async () => {
    ...
    await expect(tools.listClusters({ limit: 50 }, openSignal)).rejects.toMatchObject({
...
describe("CycleCloud mutation tools", () => {
  test("Reject pre-dispatch cancellation without calling CycleCloud", async () => {
    ...
    await expect(tools.startCluster({ clusterName: "cluster-1", recursive: false }, controller.signal)).rejects.toMatchObject({
      category: "cancelled",
    });
  ...
  test("Return accepted with one best-effort observed status", async () => {
    ...
    await expect(tools.startCluster({ clusterName: "cluster-1", recursive: true }, openSignal)).resolves.toEqual({
      action: "start",
  ...
  test("Preserve accepted when the best-effort status read fails", async () => {
    ...
    const result = await tools.terminateCluster({ clusterName: "cluster-1", recursive: false }, openSignal);

    expect(result).toMatchObject({ action: "terminate", outcome: "accepted" });
  ...
  test("Return unknown without a follow-up or retry", async () => {
    ...
    await expect(tools.startCluster({ clusterName: "cluster-1", recursive: false }, openSignal)).resolves.toEqual({
      action: "start",
      clusterName: "cluster-1",
      recursive: false,
      outcome: "unknown",
  ...
  test("Reject an overlapping mutation immediately as busy", async () => {
    ...
    const first = tools.startCluster({ clusterName: "cluster-1", recursive: false }, openSignal);
    await Promise.resolve();
    const cancelled = new AbortController();
    cancelled.abort();
    await expect(tools.terminateCluster({ clusterName: "cluster-1", recursive: false }, cancelled.signal)).rejects.toMatchObject({
      category: "cancelled",
    });
    await expect(tools.terminateCluster({ clusterName: "cluster-1", recursive: false }, openSignal)).rejects.toEqual(
      new CycleCloudRequestError("busy", true),
    );
